...
// Include secure string operations header for xpdfsearch.
#include <strsafe.h>
...
// Complete new outputToFile implementation for xpdfsearch.


static void outputToFile(void *threadData, const char *text, int len) 
{	
	ThreadData* data = (ThreadData*)threadData;	
	WaitForSingleObject (data->producerSemaphore, INFINITE);				
	BOOL found = data->found;
	if (!found)
	{
		StringCchCopyN (data->text, data->max_text_length, text, len);
		ReleaseSemaphore (data->consumerSemaphore, 1, NULL);		
	}
	else
	{
		delete data->dev;
...
// Implementation of new constructor for xpdfsearch.

TextOutputDev::TextOutputDev(ThreadData* threadData):
m_pthreadData(threadData)
{
	text = NULL;
	outputFunc = &outputToFile;
	TextOutputControl toc;
	toc.clipText = gFalse;
	toc.fixedLineSpacing = gFalse;
	toc.fixedPitch = gFalse;
	toc.html = gFalse;
	toc.mode = textOutReadingOrder;
	text = new TextPage(&toc);
	ok = gTrue;
}
...
TextOutputDev::~TextOutputDev() {
  if (needClose) {
  // Outcomment deletion of outputstream file in destructor.
...
void TextOutputDev::endPage() {
	if (m_pthreadData) 
	{
		text->write(m_pthreadData, outputFunc);
	}
}
